...
cd /groups/gcc50623/acf15833kg/tmp/Megatron-DeepSpeed
source $HOME/.bashrc

source /etc/profile.d/modules.sh
module load cuda/11.8/11.8.0 cudnn/8.9/8.9.2 hpcx/2.12 nccl/2.16/2.16.2-1
...
num_gpus=4 #$(($(ds_ssh nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)-2))
num_gpus_pernode=4 #$(nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)
...
data_path="${dir}/dataset/en_wiki_text_document"
...
vocab_path="${dir}/dataset/gpt2-vocab.json"
if [ ! -f "$vocab_path" ]; then
    wget https://s3.amazonaws.com/models.huggingface.co/bert/gpt2-vocab.json
fi
merge_path="${dir}/dataset/gpt2-merges.txt"
...
template_json="${dir}/examples_deepspeed/rebase/ds_config_gpt_TEMPLATE.json"
